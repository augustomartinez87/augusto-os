...
- **Kredy** — Next.js/tRPC/Prisma/Clerk en Vercel (`kredy-ap.vercel.app`). App de **préstamos/crédito** + capa AP (Agente Productor): amortización francesa, XIRR, multi-moneda ARS/USD, generación de mutuo + pagaré, scoring del AP y límites de originación por deudor y por vínculo. Prod en **Neon** (migrado desde Supabase `jymdblurkpadupdqzfzo` el 2026-07-08; Supabase sigue vivo sin tráfico como rollback hasta retirarlo — ver S-010 en BACKLOG.md); dev del loop también en **Neon** `kredy-dev`. (Ex repo "spensiv", renombrado 2026-06-20.)
...
- **augusto-os** — el orquestador autónomo + su memoria de sistema. Opus planifica (Arquitecto), Sonnet ejecuta (Builder), Verifier corre tsc+lint+tests, y deploya a prod solo cuando todo da verde (ADR-0019). Control remoto por Telegram (bot AlantORCH). Prod en **Supabase**, org `supabase.amartinez` (`gbztwnshsmcwqnqjouhi`) — mismo org que Argos, mismo proyecto/ref de siempre (`exyhnrynpuflbuprmdto`), movido ahí vía Project Transfer el 2026-07-08 (antes vivía en una segunda cuenta Supabase separada; ver S-035 en BACKLOG.md).
