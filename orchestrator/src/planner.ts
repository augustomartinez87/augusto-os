...
export async function planFeature(featureMd: string): Promise<PlanStep[]> {
  ...
  const prompt = `Sos el planner de un orquestador de código. Dada la siguiente spec de feature para Spensiv (Next.js/tRPC/Prisma), descomponela en pasos atómicos e implementables por un agente de código.
